// ==UserScript==
// @name         Quizlet Live Bot
// @match        quizlet.com/*
// @version      0.1
// @description  Clicks correct answers for You;
// @author       SnowLord7, forked by PetrusTryb
// @require      https://cdnjs.cloudflare.com/ajax/libs/babel-standalone/6.18.2/babel.js
// @require      https://cdnjs.cloudflare.com/ajax/libs/babel-polyfill/6.16.0/polyfill.js
// ==/UserScript==
(function() {
    let html = `
        <div style="line-height: 1.2; top: 0; left: 0; font-family: 'Courier New', Courier, monospace; z-index: 2147483647; background-color: #202020; position: fixed; width: 350px; height: 100px; margin: 0; padding: 0; box-shadow: 0 5px 35px rgba(0, 0, 0, .75)">

            <nav style="cursor: move; user-select: none; text-align: right; color: #fff; position: absolute; width: 348px; height: 25px; margin: 1px;">
                <span onclick="alert('Type Your game code in both inputs. Join the game and click Obtain Live Data to start the bot.')" style="font-size: 15px; cursor: pointer; width: 100%; height: 100%;">?</span>
            ...
            <div style="margin: 0; border-radius: 3px; background-color: #141414; position: absolute; bottom: 5px; width: 346px; height: 70px; margin: 2px;">
                ...
                <textarea spellcheck="false" id="quizletLiveDataInput" class="customScrollBar" placeholder="Paste The Site Data Here" style="display:none"></textarea>
            </div>
            <div style="margin: 0; position: absolute; bottom: 0px; width: 346px; height: 175px; margin: 2px;display:none;">
                <div style="margin: 0;height: 75px;" id="customCommandOutput" class="customScrollBar">
    ...
    const handleUpdate = () => {
        if (!input.value) {
            ...
            try {
                let jso = JSON.parse(input.value);
                let json=jso[1];
                console.log(json);
        ...
        if (code.length == 6 && !isNaN(code)) {
            ...
            .then((data) => {
                ...
                if (data.responses[0].models.gameInstance.length > 0) {
                    ...
                    let token = Quizlet.studentGameData.multiplayerToken;
                    fetch(`https://quizlet.com/multiplayer/3/45413/${code}/games/socket/?gameId=${code}&token=${token}&EIO=3&transport=polling&t=M-Gll4B`,{credentials: 'include',mode:'no-cors'}).then((r) => r.text())
            .then((d) => {
                        let sid = d.slice(d.search("\"sid\":\""),d.search(",\"upgrades\"")).split(":")[1].replace(/\"/g,"")
                        log('Got SessionId: '+sid,'#00ff00')
                        let termsEndpoint = `https://quizlet.com/multiplayer/3/45413/${code}/games/socket/?gameId=${code}&token=${token}&EIO=3&transport=polling&t=M-Gll4B&sid=${sid}`
                        fetch(termsEndpoint,{credentials: 'include',mode:'no-cors'}).then((a) => a.text())
            .then((ans) => {
                            let js=ans.substr(ans.search(/\[/),ans.length);
                            //console.log(js);
                            document.getElementById("quizletLiveDataInput").value=js;
                            log("Obtained all answers","#00FF00");
                            handleUpdate();
                        });
                    });
                } else {
        ...
        if (set && Object.keys(set).length > 0 && (window.location.href.indexOf('quizlet') && window.location.href.indexOf('live')) != -1) {
            if (document.getElementById('liveAnswerPhrase') && document.getElementsByClassName('StudentPrompt-inner')[0] && document.getElementsByClassName('StudentTerm is-clickable can-beClicked')) {
                ...
                set.terms.filter((term) => {
                    if (term.word.toLowerCase() == question.toLowerCase()) {
                        return true;
                    } else if (term.definition.toLowerCase() == question.toLowerCase()) {
                        return true;
                    }
                }).forEach(function(word) {
                    if (word.word.toLowerCase() == question.toLowerCase()) {
                        document.getElementById('liveAnswerPhrase').textContent = word.definition;
                    } else {
                        document.getElementById('liveAnswerPhrase').textContent = word.word;
                    }
                    if (!paused) {
                        for (let i = 0; i < options.length; i++) {
                            if (word.definition.toLowerCase() == options[i].innerText.trim().toLowerCase()) {
                                click(options[i]);
                            } else if (word.word.toLowerCase() == options[i].innerText.trim().toLowerCase()) {
                                click(options[i]);
                            }
                        }
                    }
